...
constexpr slice<CharT, Traits>::slice(const CharT *str) noexcept
    : length(str ? Traits::length(str) : 0)
    , data(str, length) {}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr slice<CharT, Traits>::slice(const CharT *str, size_t size) noexcept
    : length(size)
    , data(str, length) {}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr slice<CharT, Traits>::slice(view_t view) noexcept
    : length(view.size())
    , data(view) {}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr slice<CharT, Traits>::slice(char_vec &vec) noexcept
    : length(vec.size())
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
template <typename U>
constexpr slice<CharT, Traits>::slice(
    const char *str,
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr void slice<CharT, Traits>::exchange(slice &other) noexcept {
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr void slice<CharT, Traits>::replace(slice &other) noexcept {
    this->data   = other.data;
    this->length = other.length;
}
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr void slice<CharT, Traits>::replace(CharT *str, usize size) noexcept {
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr typename slice<CharT, Traits>::slice
slice<CharT, Traits>::subslice(usize pos, usize len) const noexcept {
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr typename slice<CharT, Traits>::slice
slice<CharT, Traits>::l_strip(char_vec &delim) const {
    usize start = 0;
    usize end   = length;
    while (start < end &&
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr typename slice<CharT, Traits>::slice
slice<CharT, Traits>::r_strip(char_vec &delim) const {
    usize start = 0;
    usize end   = length;
    while (end > start &&
...
template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr typename slice<CharT, Traits>::slice slice<CharT, Traits>::strip(char_vec &delim) const {
    return l_strip(delim).r_strip(delim);
}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr bool slice<CharT, Traits>::starts_with(slice &needle) const {
    return length >= needle.size() && subslice(0, needle.size()) == needle;
}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr bool slice<CharT, Traits>::ends_with(slice &needle) const {
    return length >= needle.size() && subslice(length - needle.size(), needle.size()) == needle;
}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr bool slice<CharT, Traits>::contains(slice &needle) const {
    return data.find(needle) != view_t::npos;
}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr bool slice<CharT, Traits>::contains(wchar_t &chr) const {
    return data.find(chr) != view_t::npos;
}

template <typename CharT, typename Traits>
    requires CharTraits<Traits, CharT>
constexpr isize slice<CharT, Traits>::compare(slice &other) const noexcept {
...
CharT slice<CharT, Traits>::operator[](usize index) const noexcept {
    return data[static_cast<usize::c_type>(index)];
}
