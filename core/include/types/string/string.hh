...
inline string to_string(const sstring &t) { return {t.raw(), t.size()}; }
...
template <typename Ty>
    requires std::Interface::Castable<std::Meta::all_extents_removed<Ty>, sstring>
inline string to_string(Ty &&t) {
    return t.operator$cast(static_cast<sstring *>(nullptr));
}
